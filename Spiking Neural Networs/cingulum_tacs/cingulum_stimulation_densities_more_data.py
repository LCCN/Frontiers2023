...
input2 = int(sys.argv[2])  # amplitude of the external signal fixed
...
folder_save = os.path.join(current_folder, "OzCz_densities_wstimfit2")
...
frequency = data_working_point["fpeak_cluster"].values[input1]#np.arange(4,18.1,0.25)[input3]
...
runtime  = 50000.0 # simulation time 
tstart   = 16000.0
time_cut = 4000.0   # tim
...
for ns in range(nsims):
    ...
    net.set_injected_currents_from_densities(weights_modulation, frequency,tstart)
    print('--------------')
...
time = copy.deepcopy(t[0])
window1 = (time<tstart) #baseline
window2 = (time>=tstart) #simulation
'''############################################################################
...
plv_alpha_list   = [np.zeros((npairs, nsims)),np.zeros((npairs, nsims))]
plv_alpha_matrix = [np.zeros((nnodes, nnodes, nsims)),np.zeros((nnodes, nnodes, nsims))]

for l,window in enumerate([window1,window2]):
    count = 0 
    for j1 in range(nnodes):
        for j2 in range(j1+1,nnodes):
            for k in range(nsims):
                phi1 = phase[j1,k][window]
                phi2 = phase[j2,k][window]
                plv_value = get_plv(phi1,phi2)
                print(plv_value)
                plv_alpha_list[l][count,k] = plv_value
                plv_alpha_matrix[l][j1,j2,k] = plv_value
                plv_alpha_matrix[l][j2,j1,k] = plv_value
            count+=1
###################################################################

columns = ["subject","stage","mode","trial","node", "w", "fex","fpeak", "amp_fex", "amp_fpeak","amp_norm_fex", "amp_norm_fpeak", "plv_mean","plv_std"] 

rows = dict.fromkeys(columns)
for key in columns:
    rows[key] = []

fpeak_matrix            = [ np.zeros((nnodes,nsims)), np.zeros((nnodes,nsims)) ]
power_fex_matrix        = [ np.zeros((nnodes,nsims)), np.zeros((nnodes,nsims)) ]
power_fpeak_matrix      = [ np.zeros((nnodes,nsims)), np.zeros((nnodes,nsims)) ]
power_norm_fex_matrix   = [ np.zeros((nnodes,nsims)), np.zeros((nnodes,nsims)) ]
power_norm_fpeak_matrix = [ np.zeros((nnodes,nsims)), np.zeros((nnodes,nsims)) ]

for l, (window,stage) in enumerate( zip([window1,window2],["baseline","stimulation"]) ):
    for k, ns in enumerate(range(nsims)):
        ...
        for j in range(nnodes):
            ...
            lfp = np.array(series[f"lfp_{j}_{k}"])[window]
            ...
            fpeak_matrix[l][j,k] = fpeak 
            power_fex_matrix[l][j,k] = amplitude_fex
            power_fpeak_matrix[l][j,k] = amplitude_fpeak
            power_norm_fex_matrix[l][j,k] = amplitude_norm_fex
            power_norm_fpeak_matrix[l][j,k] = amplitude_norm_fpeak

            rows["plv_mean"].append( np.mean(np.delete( plv_alpha_matrix[l][j,:,k], j)) )
            rows["plv_std"].append( np.std(np.delete( plv_alpha_matrix[l][j,:,k], j)) )

parent_dir = os.path.join(folder_save, subjects[input1])
...
#dfnew.to_csv(os.path.join(parent_dir, title),sep="\t")

columns = ["subject","stage","mode","trial", "w", "fex", "fpeak", "fpeak_std", "plv_mean","plv_std", 
...
for l,stage in enumerate(["baseline","stimulation"]):
    ...
    for k, ns in enumerate(range(nsims)):
        ...
        rows["fpeak"].append( np.mean( fpeak_matrix[l][idcb,k] ))
        rows["fpeak_std"].append(  np.std(  fpeak_matrix[l][idcb,k] ))
        
        rows["plv_mean"].append( np.mean( plv_alpha_list[l][idcb,k] ))
        rows["plv_std"].append(  np.std(  plv_alpha_list[l][idcb,k] ))
        
        rows["amp_fpeak"].append( np.mean( power_fpeak_matrix[l][idcb,k] ))
        rows["amp_fpeak_std"].append(   np.std( power_fpeak_matrix[l][idcb,k] ))
        rows["amp_norm_fpeak"].append( np.mean( power_norm_fpeak_matrix[l][idcb,k] ))
        rows["amp_norm_fpeak_std"].append(   np.std( power_norm_fpeak_matrix[l][idcb,k] ))
        ...
        rows["amp_fex"].append(    np.mean( power_fex_matrix[l][idcb,k] ))
        rows["amp_fex_std"].append( np.std( power_fex_matrix[l][idcb,k] ))
        rows["amp_norm_fex"].append(    np.mean( power_norm_fex_matrix[l][idcb,k] ))
        rows["amp_norm_fex_std"].append( np.std( power_norm_fex_matrix[l][idcb,k] ))

title = f"stimulation_cluster_{subjects[input1]}_{input2}_{input3}_{input4}.lzma"
dfnew2 = pd.DataFrame( rows, columns=columns)
file_management.save_lzma( dfnew2, title, parent_dir = parent_dir)
