...
folder_save = os.path.join(current_folder, "OzCz_densities")
...
coupling_factor = 0.0#data_working_point["coup"].values[input1]
...
for k, ns in enumerate(range(nsims)):
    ...
    for j in range(nnodes):
        ...
        rows["plv_mean"] =  np.mean(np.delete( plv_alpha_matrix[j,:,k], j)) 
        rows["plv_std"]  =  np.std(np.delete( plv_alpha_matrix[j,:,k], j)) 
